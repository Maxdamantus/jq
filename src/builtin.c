...
static jv f_dump(jq_state *jq, jv input) {
  if (jv_get_kind(input) == JV_KIND_STRING &&
      jv_get_string_kind(input) == JV_STRING_KIND_BINARY_BYTEARRAY)
...
static jv f_length(jq_state *jq, jv input) {
  if (jv_get_kind(input) == JV_KIND_ARRAY) {
    ...
    if (jv_get_string_kind(input) == JV_STRING_KIND_UTF8)
...
static jv f_error(jq_state *jq, jv input) {
  if (jv_get_kind(input) == JV_KIND_STRING &&
      jv_get_string_kind(input) != JV_STRING_KIND_UTF8) {
    return jv_invalid_with_msg(f_tostring(jq, input));
  }
...
static jv f_encodeas(jq_state *jq, jv a, jv b) {
  if (jv_get_kind(a) != JV_KIND_STRING) {
    jv_free(b);
    return a;
  }
  if (jv_get_kind(b) != JV_KIND_STRING) {
    jv_free(a);
    return type_error(b, "encodeas encoding is not a string");
  }
  const char *as = jv_string_value(b);
  if (strcmp(as, "UTF-8") == 0 || strcmp(as, "utf-8") == 0) {
    if (jv_get_string_kind(a) == JV_STRING_KIND_UTF8) {
      jv_free(b);
      return a;
    }
    a.subkind = JV_STRING_KIND_BINARY_UTF8;
  } else if (strcmp(as, "base64") == 0) {
    a.subkind = JV_STRING_KIND_BINARY;
  } else if (strcmp(as, "bytearray") == 0) {
    a.subkind = JV_STRING_KIND_BINARY_BYTEARRAY;
  } else {
    jv_free(a);
    return type_error(b, "value encoding option not supported");
  }
  jv_free(b);
  return a;
}
...
static jv f_encoding(jq_state *jq, jv a) {
  if (jv_get_kind(a) != JV_KIND_STRING)
    return type_error(a, "is not a string");
  jv_string_kind k = jv_get_string_kind(a);
  jv_free(a);
  switch (k) {
  case JV_STRING_KIND_UTF8:
  case JV_STRING_KIND_BINARY_UTF8: return jv_string("UTF-8");
  case JV_STRING_KIND_BINARY: return jv_string("base64");
  case JV_STRING_KIND_BINARY_BYTEARRAY: return jv_string("bytearray");
...
#include "libm.h"
  ...
  {(cfunction_ptr)f_encodeas, "encodeas", 2},
  {(cfunction_ptr)f_stringtype, "stringtype", 1},
  {(cfunction_ptr)f_encoding, "encoding", 1},
};
#undef LIBM_DDDD_NO
#undef LIBM_DDD_NO
#undef LIBM_DD_NO
